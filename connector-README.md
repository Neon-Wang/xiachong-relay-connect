...
> Pinit 桌面宠物与 OpenClaw 之间的中继连接层。当前主路径是 **OpenClaw Channel Plugin**；Python CLI Connector 仍保留为不能安装插件时的兼容 / 保底路径。

## 两种模式：选哪个？

| 维度 | CLI 模式（本 README） | Channel 插件模式（[`channel-plugin/`](./channel-plugin/)） |
|---|---|---|
| 包 | `pinit-relay-connect`（**npm PENDING**，目前用 R2/curl 发） | `@pinit/channel`（**npm PENDING**，目前用 R2/`openclaw plugins install ./xxx.tgz` 发） |
...
两条路径**共用同一个 Workers relay 和 Electron 客户端**，切换时不需要迁移凭证：插件首次运行会自己跑 `/api/link` 再持久化到 `~/.openclaw/channels/pinit/state-default.json`。

> **npm 通道当前 PENDING**（2026-04-22 起）：
> - `pinit-relay-connect` 始终未在 npm 注册成功，[`RELEASE.md`](./RELEASE.md) 是历史 npm 发版手册，**别按它操作**。
> - `@pinit/channel` 同样未发到 npm，channel-plugin 的 CI publish step 已注释掉。
...
| 路径 | 职责 |
|---|---|
| `pinit-connect.py` | Python CLI connector 主体：relay 配对、WebSocket 循环、OpenClaw CLI subprocess 调用、回复解析 |
| `bin/run.js` | npm bin 包装入口；包名恢复后暴露为 `pinit-relay-connect` |
| `package.json` | npm 包元数据（当前 npm 发布 pending，`bin` 名为 `pinit-relay-connect`） |
...
`pinit-connect.py` 是一个**纯文本聊天消息转发器**。它的完整工作流程如下：
...
- 不读写本地文件（除了 `~/.config/pinit/agent.json` 凭证文件和标准输出日志）
...
| 命令 | 调用条件 | 副作用 |
|---|---|---|
| `python3 -u pinit-connect.py --relay X --link-code Y --secret Z` | 默认启动方式；进入 connector 消息转发主循环。`--link-code` / `--secret` 仅首次配对时使用，后续 connector 会用 `~/.config/pinit/agent.json` 里保存的 `agent_token` 自动认证。npm 通道恢复后等价入口是 `pinit-relay-connect ...`。 | 写入 `agent.json`（首次），与 relay 建立 WebSocket 长连接，收到消息时 `subprocess` 拉起 `openclaw agent` 子进程 |
| `python3 -u pinit-connect.py status` | 排查问题时快速诊断：输出 relay 配对状态、OpenClaw CLI 是否在 PATH 中。npm 通道恢复后等价入口是 `pinit-relay-connect status`。 | 仅读取，无副作用。Exit code 0 = OK；1 = OpenClaw CLI 不可用（connector 仍能启动但只能 echo） |
...
```bash
python3 -u pinit-connect.py status
```
...
| `--agent-file` | 否 | Agent 凭证文件路径（默认: `~/.config/pinit/agent.json`） |
...
```mermaid
flowchart TB
  Client["Pinit Client"] <-->|"WebSocket"| Relay["Cloudflare Workers Relay"]
  Relay <-->|"WebSocket"| Connector["pinit-connect.py"]
...
1. 用 Pinit 客户端给的 Link Code + Secret 绑定到中转服务器（首次）；后续用保存的 `agent_token` 自动认证
...
1. **首次配对**：使用客户端提供的 `link_code` + `secret` 完成绑定，生成 `agent_token` 保存到 `~/.config/pinit/agent.json`
...
- 如需重新配对，删除 `~/.config/pinit/agent.json` 后重启
...
```bash
# 1. 下载脚本（已有则跳过）
curl -sO https://primo.evomap.ai/connector/pinit-connect.py
...
- **配置文件读写** `load_agent_file` / `save_agent_file`：仅访问 `~/.config/pinit/agent.json` 这一个固定路径
...
`connector/scripts/e2e-test-client.py` 扮演 Pinit 客户端，自动 register + 连 `/ws/client` + 发消息 + 收 reply，可独立于真实客户端验证整条链路。
...
# （见 pinit-relay.service 模板或 nohup 直接启动）
...
Electron 客户端自带一个 dev-only test HTTP 服务（`client/electron/test-server.js`，默认 `127.0.0.1:11453`），能直接吐当前 link_code / secret，免得手抄 UI。调用条件：`pnpm electron:dev` 在跑，Bearer token 在 `~/Library/Application Support/Pinit/test-server-token.txt`（macOS）或 `~/.config/Pinit/test-server-token.txt`（Linux）。

```bash
# 1) 启 Electron dev（后台）
pnpm --filter pinit-client electron:dev &

# 2) 读 token、拿 credentials
TOKEN=$(cat "$HOME/Library/Application Support/Pinit/test-server-token.txt")
...
# python3 -u connector/pinit-connect.py --relay https://primo.evomap.ai \
#     --link-code ABCDEF --secret <the-secret> \
#     --agent-file /tmp/agent.json
# (历史命令，npm 重启后再恢复:)
# npx pinit-relay-connect --relay https://primo.evomap.ai \
#   --link-code ABCDEF --secret <the-secret> \
#   --agent-file /tmp/agent.json
...
`connector/scripts/pinit-relay.service` 是生产环境部署参考。使用方式：

```bash
mkdir -p ~/.config/pinit
cat > ~/.config/pinit/relay.env <<'EOF'
PINIT_RELAY_URL=https://primo.evomap.ai
PINIT_LINK_CODE=你的LINK_CODE
PINIT_SECRET=你的SECRET
EOF
chmod 600 ~/.config/pinit/relay.env

cp connector/scripts/pinit-relay.service ~/.config/systemd/user/
systemctl --user daemon-reload
systemctl --user enable --now pinit-relay.service
```
...
本目录是 [Pinit monorepo](https://github.com/EvoMap/Pinit) 的子项目，推送到 `main` 分支时自动同步到公开镜像 [Neon-Wang/xiachong-relay-connect](https://github.com/Neon-Wang/xiachong-relay-connect)。

> **历史名说明**：GitHub 仓库名 `xiachong-relay-connect` 是历史代号，重命名会破坏所有已有的 git remote / fork，因此保留。npm 包名使用 `pinit-relay-connect`。
>
...
> 2. 或者 `curl -sO https://primo.evomap.ai/connector/pinit-connect.py` 拿单文件
>
