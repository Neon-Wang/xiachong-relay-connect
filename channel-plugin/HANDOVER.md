...
# Staging（dev 联调）
curl -fL -o pinit-channel.tgz \
  https://xiachong-api-staging.aged-sea-ee35.workers.dev/channel-plugin/latest.tgz
```
...
| `https://xiachong-api-staging.aged-sea-ee35.workers.dev/channel-plugin/...` | 同上 | Staging 镜像，dev 联调用 |
...
# Staging
curl -fsSL https://xiachong-api-staging.aged-sea-ee35.workers.dev/health
curl -fsSL https://xiachong-api-staging.aged-sea-ee35.workers.dev/channel-plugin/latest.json | jq .
...
| `xiachong-api-staging.aged-sea-ee35.workers.dev` | Custom domain → staging Workers（dev/联调用） |

> 客户端 `client/.env.development` 永远指 `xiachong-api-staging.aged-sea-ee35.workers.dev`（staging）。客户端生产 build 走 `pinit-api.aged-sea-ee35.workers.dev`。
...
- 客户端 `client/.env.development` 写死 `https://xiachong-api-staging.aged-sea-ee35.workers.dev`，**永不改**
