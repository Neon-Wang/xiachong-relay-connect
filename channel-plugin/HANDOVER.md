...
> **写这份文档的目的**：让任何新接手 Pinit channel plugin 的人（或 AI agent）在 30 分钟内能完成"端到端连一遍"，并在踩坑时知道去哪查。
>
...
| 你想做什么 | 跳到 |
|---|---|
| **首次拉起 channel plugin 让 Pinit 客户端能聊天** | [§1 端到端连接流程](#1-端到端连接流程稳态版本) |
...
> 描述用户从零开始，让 Pinit 桌面客户端通过 channel plugin 与 OpenClaw 完整跑通的完整路径。中间的每一步都已通过 VM 实测（2026-04-22，Debian 12 + OpenClaw 2026.4.5）。
...
[Cloudflare Workers Relay]
    ...
    │
[OpenClaw Gateway]
    │   ③ 装上 @pinit/channel 插件（tarball 来自 R2 或 GitHub Release）
    │   ④ 配置 ~/.openclaw/openclaw.json 的 channels.pinit
    │   ⑤ openclaw gateway start → plugin 自动 pair + 建立 WebSocket
    ▼
[Pinit 客户端] 发消息 → Workers → plugin → OpenClaw agent → 反向回流
```
...
curl -fL https://pinit-api.aged-sea-ee35.workers.dev/channel-plugin/latest.sha256 \
  | shasum -a 256 -c -
# 输出：pinit-channel.tgz: OK

# Staging（dev 联调）
curl -fL -o pinit-channel.tgz \
  https://pinit-api-staging.aged-sea-ee35.workers.dev/channel-plugin/latest.tgz
```
...
VERSION="0.1.1"
curl -fL -o pinit-channel.tgz \
  "https://github.com/EvoMap/Pinit/releases/download/channel-plugin-v${VERSION}/pinit-channel-${VERSION}.tgz"
curl -fL -o latest.sha256 \
  "https://github.com/EvoMap/Pinit/releases/download/channel-plugin-v${VERSION}/latest.sha256"
shasum -a 256 -c latest.sha256
```

#### 腿 C（PENDING）：~~`openclaw plugins install @pinit/channel`~~
...
> 需要 OpenClaw `>=2026.4.0`。2026.3.x 缺少本插件依赖的 `channel-core` / `channel-contract` SDK 入口，手动解压 tarball 也会在 Gateway 加载时报 `plugin-sdk/root-alias.cjs/channel-core`。旧版本用户请升级 OpenClaw 或暂时走 CLI Connector 兜底。

> ⚠️ **0.1.0 重装陷阱**：如果之前装过 0.1.0 preview 又把 `~/.openclaw/extensions/pinit/` 手动删了，但 `openclaw.json` 还留着 `channels.pinit`，会报 `Invalid config: channels.pinit: unknown channel id`。处置脚本见 [`README.md › Re-install gotcha`](./connector-channel-plugin-README.md#re-install-gotcha)。
...
| dev build (`pnpm electron:dev`) | Electron 主进程会把这对凭证打到 stderr：`grep -E '\[pinit\] (linkCode|secret)='` |
...
{
  "plugins": {
    "allow": ["pinit"]   // 强烈推荐：杜绝意外加载第三方插件
  },
  "channels": {
    "pinit": {
      "relayUrl": "https://pinit-api.aged-sea-ee35.workers.dev",
      "linkCode": "ABC123",
...
```text
[pinit] started account default (relay=https://pinit-api.aged-sea-ee35.workers.dev)
[pinit] pinit-ws: pairing: trying agent-auth with stored agent_token
[pinit] paired via agent-auth (appId=openclaw_xxxx agentId=agent_xxxx)
   # 或首次：paired via link (...)
...
|---|---|---|
| `https://pinit-api.aged-sea-ee35.workers.dev/channel-plugin/latest.tgz` | `max-age=300`（5 分钟） | 跟着最新版变 |
| `https://pinit-api.aged-sea-ee35.workers.dev/channel-plugin/v<semver>.tgz` | `max-age=31536000, immutable` | 永不变 |
| `https://pinit-api.aged-sea-ee35.workers.dev/channel-plugin/latest.sha256` | `max-age=60` | 跟着最新版变 |
| `https://pinit-api.aged-sea-ee35.workers.dev/channel-plugin/latest.json` | `max-age=60` | 跟着最新版变 |
| `https://pinit-api-staging.aged-sea-ee35.workers.dev/channel-plugin/...` | 同上 | Staging 镜像，dev 联调用 |
...
curl -fI https://pinit-api.aged-sea-ee35.workers.dev/channel-plugin/v0.1.2.tgz
curl -fsSL https://pinit-api.aged-sea-ee35.workers.dev/channel-plugin/latest.json | jq .version

# 3) GitHub Release 在
gh release view channel-plugin-v0.1.2 --repo EvoMap/Pinit

# 4) 在 VM smoke
ssh xc-debian "
  curl -fL -o /tmp/p.tgz https://pinit-api.aged-sea-ee35.workers.dev/channel-plugin/latest.tgz && \
  openclaw plugins install /tmp/p.tgz && \
...
```bash
# Production
curl -fsSL https://pinit-api.aged-sea-ee35.workers.dev/health
curl -fsSL https://pinit-api.aged-sea-ee35.workers.dev/channel-plugin/latest.json | jq .

# Staging
curl -fsSL https://pinit-api-staging.aged-sea-ee35.workers.dev/health
curl -fsSL https://pinit-api-staging.aged-sea-ee35.workers.dev/channel-plugin/latest.json | jq .

# 防御回归：恶意路径必须 400
curl -i "https://pinit-api.aged-sea-ee35.workers.dev/channel-plugin/../../../etc/passwd"
...
| `.github/workflows/publish-connectors.yml` | push 到 `main` 改 `connector/**`（除 channel-plugin） | sync 到 `Neon-Wang/xiachong-relay-connect` + R2 上传 `pinit-connect.py`。**npm publish step 已注释**（§6） |
...
| 文件 | 角色 |
|---|---|
| `connector/channel-plugin/package.json` | 包名 `@pinit/channel`、版本号、`bin`/`exports`、`files`（决定 tarball 内容） |
| `connector/channel-plugin/openclaw.plugin.json` | OpenClaw plugin manifest（id `pinit`、`configSchema`） |
...
| 域名 | 指向 |
|---|---|
| `pinit-api.aged-sea-ee35.workers.dev` | Workers 默认域，**生产环境唯一对外入口** |
| `pinit-api-staging.aged-sea-ee35.workers.dev` | Workers 默认域，staging |
| `pinit-api-staging.aged-sea-ee35.workers.dev` | Custom domain → staging Workers（dev/联调用） |

> 客户端 `client/.env.development` 永远指 `pinit-api-staging.aged-sea-ee35.workers.dev`（staging）。客户端生产 build 走 `pinit-api.aged-sea-ee35.workers.dev`。
...
不写会让 `openclaw security audit` 报 CRITICAL `channels.pinit.dm.open`。
...
如果用户先装了 0.1.0 preview，又把 `~/.openclaw/extensions/pinit/` 手动删了（但 `openclaw.json` 还留 `channels.pinit`），下次 `openclaw plugins install` 会失败：

```text
Invalid config: channels.pinit: unknown channel id: pinit
```

**为什么**：OpenClaw `plugins install` 在写新 plugin 文件**之前**先做配置校验。配置里引用了 `pinit` 但插件目录已经被删，校验过不去。

**处置**：先临时移除 `channels.pinit`、装完再加回来。完整脚本见 [`README.md › Re-install gotcha`](./connector-channel-plugin-README.md#re-install-gotcha)。

### 5.5 `agent_token` 持久化位置（升级要兼容）

| 版本 | 位置 |
|---|---|
| 0.1.0 preview | `~/.openclaw/extensions/pinit/state-<account>.json` |
| 0.1.1+ | `~/.openclaw/channels/pinit/state-<account>.json`（mode 0600） |
...
- 不要在 plugin 升级时清空 `~/.openclaw/channels/pinit/`——会强制所有用户重新配对
...
- 客户端 `client/.env.development` 写死 `https://pinit-api-staging.aged-sea-ee35.workers.dev`，**永不改**
- 客户端生产 build 写 `https://pinit-api.aged-sea-ee35.workers.dev`
...
>
> 当且仅当出现以下场景之一：
> 1. 有多个外部团队反馈 `openclaw plugins install @pinit/channel` 是他们流程的硬依赖（curl + tarball 流程不接受）
> 2. 我们要用 `npx pinit-channel-attack-sim` 这类 npm bin 当公开诊断工具
...
| 责任人 | 做什么 | 输出 |
|---|---|---|
| 持有 `@pinit` npm scope 的同事 | 在 npm 网站确认 scope 还在自己名下、2FA 已开 | `npm whoami` 输出账号名，`npm access ls-packages` 看到 `@pinit/*` |
| 仓库 admin（Neon Wang） | 准备 npm Trusted Publishing 绑定参数 | publisher: GitHub Actions / org: EvoMap / repo: Pinit / workflow: `publish-channel-plugin.yml` |
...
cd Pinit/connector/channel-plugin
pnpm install
pnpm ci
pnpm pack --dry-run   # 检查 tarball 内容
npm login              # 持有 @pinit scope 的账号
npm publish --access public
```

`pinit-relay-connect` 同理（在 `connector/` 目录跑），按 `connector/NPM_ONBOARDING.md` 操作。

### 6.4 绑定 Trusted Publishing

到 https://www.npmjs.com/package/@pinit/channel → Settings → Trusted Publishing：
...
`pinit-relay-connect` 同理，workflow filename 填 `publish-connectors.yml`。
...
# 验证 npm 上有了
npm view @pinit/channel version    # 期望: 0.1.2
```
...
- [ ] `connector/channel-plugin/README.md` Verify it yourself 段 → 恢复 `npx -p @pinit/channel pinit-channel-attack-sim`
...
| 排查 | 处置 |
|---|---|
| `curl -fI https://pinit-api.../channel-plugin/latest.tgz` 看 `etag` | 和 `latest.json.sha256` 对比 |
...
### 7.3 `openclaw plugins install` 失败：`Invalid config: channels.pinit: unknown channel id`
...
```bash
# 收集证据
journalctl --user -u openclaw-gateway -n 200 | grep pinit > /tmp/log.txt
...
| 原因 | 处置 |
|---|---|
| 客户端那边重新生成了 linkCode | 用新凭证、删 `~/.openclaw/channels/pinit/state-default.json` 强制重 link |
...
| 原因 | 处置 |
|---|---|
| 公司防火墙 / 路由器 / 透明代理拦 WebSocket upgrade | `curl -fI https://pinit-api.../health` 测 HTTPS 通；用手机热点对比 |
...
| `@pinit` npm scope 持有 | （PENDING 状态下无人) | 重启 npm 通道时再确定 |
| 安全问题上报 | `security@evomap.ai` | 不公开开 issue |
| 一般 bug / 功能请求 | https://github.com/EvoMap/Pinit/issues | 公开 |
